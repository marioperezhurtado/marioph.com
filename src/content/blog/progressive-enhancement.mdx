...
```tsx title="todos.tsx" /preventDefault()/ /onSubmit/ 
...
```tsx title="my-accessible-dropdown.tsx"
...
```svelte title="todos.svelte" /create/
...
```ts title="+page.server.ts" /create:/
...
```svelte title="todos.svelte" {2,3,8,14-21,26,35-37}
<script lang="ts">
