...
- For very large conferences, the number of streams per core may become a limitation. In such
  cases, the `pipeToRouter` feature can be used to interconnect `Routers` running in separate
...
In this post, we explored the fundamentals of WebRTC, the limitations of peer-to-peer,
