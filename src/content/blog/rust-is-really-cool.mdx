...
```rust title="main.rs" /mut bar/ /mut buzz/
let foo = 5;
...
```rust title="read_file.rs" /Result<String, Error>/
use std::fs;
use std::io::Error;
...
```typescript title="error_handling.ts" /err: ???/
try {
...
```rust title="find_haystack.rs" /Option<usize>/
